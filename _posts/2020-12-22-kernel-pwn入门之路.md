...
- misc_register()用于注册一个驱动，其参数为`miscdevice`结构体指针
    ...
    >内核在加载驱动的时候，调用`module_init()`函数，`module_init()`函数再调用`misc_register()`来向内核注册驱动，将驱动程序的`file_operations`结构体连同其主设备号一起传入内核
    >
    >file_operations结构体定义为：
    >
    >![](https://tva1.sinaimg.cn/large/0081Kckwly1glwm6vhgqdj316i0u0h0p.jpg)
    >
    >可以通过修改其中的函数指针来达到重写某个函数的目的，如果对这个驱动调用某个其中的函数，就会调用结构体中的函数指针
    >
    >
    >
    >举个栗子
